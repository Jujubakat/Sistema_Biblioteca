	...
	$("#formCadastroLivro").submit(function(event){
		...
		request.done(function (response, textStatus, jqXHR){
			...
			if (!response.success) { //Se deu alguma mensagem de erro
				...
				if (response.erros.autor) {
					$('#divMensagem').append('<div class="alert alert-danger" role="alert">' + response.erros.autor + '</div>')
					.fadeIn(1000).html();
				}
				if (response.erros.categoria) {
					$('#divMensagem').append('<div class="alert alert-danger" role="alert">' + response.erros.categoria + '</div>')
					.fadeIn(1000).html();
				}			
			}
			else {
